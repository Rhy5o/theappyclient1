    ...
    func paymentQueue(_ queue: SKPaymentQueue, updatedTransactions transactions: [SKPaymentTransaction])
    {
        for transaction in transactions
        {
            ...
            case SKPaymentTransactionState.purchased:
                ...
                webView.evaluateJavaScript("var transactionIdentifier = '\(transactionIdentifier)';") { result, error in
                    if let error = error {
                        print("Error setting transactionIdentifier in JavaScript: \(error.localizedDescription)")
                    }
                }

                // Sicheres Abrufen der Produkt-ID
                let safeProductId = safeString(productId)
                webView.evaluateJavaScript("var planID = '\(safeProductId)';") { result, error in
                    if let error = error {
                        print("Error setting planID in JavaScript: \(error.localizedDescription)")
                    }
                }
                ...
                if let receiptURL = Bundle.main.appStoreReceiptURL,
                   FileManager.default.fileExists(atPath: receiptURL.path) {
                    do {
                        ...
                        let receiptString = safeString(receiptData.base64EncodedString())
                        print("Receipt Data: \(receiptString)")

                        webView.evaluateJavaScript("var subreceipts = '\(receiptString)';") { result, error in
                            if let error = error {
                                print("Error setting subreceipts in JavaScript: \(error.localizedDescription)")
                            }
                        }
                    } catch {
            ...
            if let receiptURL = Bundle.main.appStoreReceiptURL,
               FileManager.default.fileExists(atPath: receiptURL.path) {
                do {
                    ...
                    let productId = safeString(purchase.productId ?? "n/a")
                    ...
                    webView.evaluateJavaScript("var planID = '\(productId)';") { result, error in
                        if let error = error {
                            print("Error setting planID in JavaScript: \(error.localizedDescription)")
                        }
                    }

                    webView.evaluateJavaScript("var subreceipts = '\(receiptStringSafe)';") { result, error in
                        if let error = error {
                            print("Error setting subreceipts in JavaScript: \(error.localizedDescription)")
                        }
                    }
